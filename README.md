...
#### Data Dictionary
        ...
        payment_type_id                        Unique payment type id for customer                            Int 
        
        internet_service_type_id               Unique internet service type id for customer                   int 
        
        contract_type_id                       Unique contract type id for customer                           int 
        
        senior_citizen                         1 if customer is senior citizen                                int
        
        tenure                                 Months of tenure as a customer                                 int  
        
        monthly_charges                        Customer's monthly bill charges                                float
        
        total_charges                          Customer's total bills                                         float
        
        gender_encoded                         Yes for 1 if customer is female                                int 
        
        partner_encoded                        Yes for 1 if customer is partner                               int 
        
        dependents_encoded                     Yes for 1 if customer has dependents                           int 
        
        phone_service_encoded                  Yes for 1 if customer has phone service                        int
        
        paperless_billing_encoded              Yes for 1 if customer has paperless billing                    int
        
        multiple_lines_No phone service        Yes for 1 if customer has multiple_lines but no phone service  int
        
        multiple_lines_Yes                     Yes for 1 if customer has multiple_lines                       int
        
        online_security_No internet service    1 if customer has internet but no online security              int 
        
        online_security_Yes                    1 if customer has online security                              int
        
        online_backup_No internet service      1 if customer has online_backup but no internet service        int
        
        online_backup_Yes                      1 if customer has online_backup                                int
        
        device_protection_No internet service  1 if customer has internet but no device protection            int

        device_protection_Yes                  1 if customer has device protection                            int

        tech_support_No internet service       1 if customer has tech_support but no internet service         int

        tech_support_Yes                       1 if customer has tech_support                                 int

        streaming_tv_No internet service       1 if customer is streaming_tv but no internet service          int

        streaming_tv_Yes                       1 if customer is streaming_tv                                  int

        streaming_movies_No internet service   1 if customer is streaming_movies but no internet service      int

        streaming_movies_Yes                   1 if customer is streaming_movies                              int 

        contract_type_One year                 1 if customer has contract_type_One year                       int

        contract_type_Two year                 1 if customer has contract_type_Two year                       int

        internet_service_type_Fiber optic      1 if customer has internet_service_type_Fiber optic            int

        internet_service_type_None             1 if customer has No internet_service_type                     int

        payment_type_Credit card (automatic)   1 if customer has automatic payment_type_Credit card           int
        
        payment_type_Electronic check          1 if customer has Electronic check payment_type                int
        
        payment_type_Electronic check          1 if customer has Electronic check payment_type                int 
        
        payment_type_Mailed check              1 if customer has Mailed check payment_type                    int
